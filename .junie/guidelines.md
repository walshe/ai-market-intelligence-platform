...
* Place all migration scripts in the default directory: `src/main/resources/db/migration`.
* Follow the standard naming convention for migration files: `V<Version>__<Description>.sql` (e.g., `V1__init_database.sql`). Note the double underscore between the version and the description.
...
project will use springdoc-openapi-starter-webmvc-ui to expose a swagger ui
...
Do NOT implement features outside the current phase defined in MASTER-PLAN.md.
...
## 17.3 LLM Integration Rules

- All LLM calls must go through the `LlmClient` interface.
- No direct OpenAI SDK calls in controllers.
- Controllers must remain thin.
- Prompt construction must be handled by `PromptBuilderService`.
- Cost tracking must be handled by `CostTrackingService`.
...
- Avoid heavy NLP libraries.
- Keep chunking deterministic and simple.
- Embeddings must be stored at chunk level, not document level.

---

## 17.5 Service Layer Separation

- Controllers must not contain business logic.
- Retrieval logic must remain separate from prompt construction.
- Embedding generation must not occur inside controllers.
- Do not introduce unnecessary abstracti
